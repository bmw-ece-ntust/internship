FAPI interface OAI L1/L2
========================
FAPI procedures are split into two groups: configuration procedures and slot procedures. 

FAPI initialization procedure

context : 
The initialization procedure moves the PHY from the IDLE state to the RUNNING state,
via the CONFIGURED state.



there are 3 stages of FAPI initialization
- The PARAM message exchange procedure
- The CONFIG message exchange procedure
- The START message exchange procedure

![image](https://github.com/bmw-ece-ntust/internship/assets/123167913/df069bfc-8a12-43ac-8cbb-632693c80f04)

For SFN/SL Synchronization, the initialization procedure is completed when the PHY
sends the L2/L3 software a SLOT.indication message. (keyword :  SLOT.indication)

For Delay Management with Timestamps, the initialization procedure is completed
when the PHY sends the L2/L3 software a START.response message.

For Delay Management without Timestamps, the initialization procedure is completed
when the PHY sends the L2/L3 software a TIMING.indication message.


In OAI, the data neccessary for NR_UL_indiation function can be found below, where this struct is the input for the whole NR_UL_indication function
...
In OAI, the data regarding PARAM, CONFIG and START and movement between states  is a struct with the following structure 
...
In OAI, the control of NR_PHY_Interface can be found at NR_IF_module where this module contains all function and parameter definition neccesary for NR_UL_indication block diagram 

![image](https://github.com/bmw-ece-ntust/internship/assets/123167913/fd96798a-2fdb-4f25-8e3b-7b923afcf5a9)
...
```
the function handle_nr_RACH is responsible for extracting random access channel infromation such as number of PDU, SFN and slot from UL_INFO struct

while handle_nr_uci extract information sfn,slot,controlLength, number of UCI from UL_INFO struct


On the other hand, the UL-SCH transport channel is used to send data from the UE to the gNB. HARQ is
always applied on the UL-SCH transport channel and the ACK/NACK value is indicated when the next transmission for the HARQ process is scheduled via DCI.

The following is the prodecure for UL_SCH procedure

![image](https://github.com/bmw-ece-ntust/internship/assets/123167913/db14cb12-498f-4bf8-afd3-65263144c1bd)
