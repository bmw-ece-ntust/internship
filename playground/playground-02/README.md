...
1. **Open5GS:** Subscriber database needs to be updated with more UEs. Use given "subscriber_db.csv" file.
...
2. **gNB:** Use modified gnb config to operate with a channel bandwidth of 10 MHz and use the ZMQ-based RF driver.
...
3. **srsUE:** srsUE config files were modified to operate with a channel bandwidth of 10 MHz and use the ZMQ-based RF driver. In addition, the config files were modified to allow the execution of multiple srsUE processes on the same host PC. Specifically, each config file has different: ports for ZMQ connections, pcap/logs filename, imsi, network namespace.
...
4. **GNU-Radio Companion:**  Run the GRC Flowgraph associated with the broker. Once started, clock "Execute the flow graph".
